...
const playAndPause = () => {
  ...
  if(audio.src.substring(audio.src.length - 3) == 'mp3'){
    //inputRange
    document.getElementById('inputMusic').max = parseInt(audio.duration) 
    ...
    const timer = () => {
      ...
      document.getElementById('currentDuration').innerText = m + ':' + s
      s++
      
      document.getElementById('inputMusic').value = currentInputPercentage
      currentInputPercentage++
    }
    if(startTimer == false){
    ...
    //totalTime
    document.getElementById('totalDuration').innerText = Math.floor((parseInt(audio.duration) / 60)) + ':' + Math.floor((audio.duration) % 60)
  }
}
